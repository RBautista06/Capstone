...
<html>
<head>
    <title>Interment Reschedule Payment</title>
...
<script>
    function navigatePage() {
        ...
        if (interment.checked) {
            window.location.href = 'frontdesk_reschdule_payment.php?Id=<?php echo urlencode($_GET['Id']); ?>';  // Replace with the actual URL for the INTERMENT page
        } else if (transferOfRights.checked) {
            window.location.href = 'frontdesk_tor_reschedule_payment.php?Id=<?php echo urlencode($_GET['Id']); ?>';  // Replace with the actual URL for the TRANSFER OF RIGHTS page
        }
    }
</script>
...
$sql = "SELECT * FROM tor_proof_of_payments WHERE STATUS = 'reschedule payment'";
...
while ($row = mysqli_fetch_assoc($result)) {;
    $intermentorederid = $row['TOR_ID'];
    $customerNameQuery = "SELECT * FROM transfer_of_rights WHERE ID = '$intermentorederid'";
    ...
    $customerName = $customerNameRow['PAYMENT_OPTION'];
                        ...
                        echo "
                        <tr class='clickable-row' data-href='reschedule_tor_paymentdetails.php?id=" . $row['PROOF_ID'] . "&account_id=" . $account_id . "&intermentid=".$row['TOR_ID']."'>
                            <td>" . htmlspecialchars($row['PROOF_ID']) . "</td>
                            <td>" . htmlspecialchars($row['TOR_ID']) . "</td>
                            <td>" . htmlspecialchars($row['PREPARED_BY']) . "</td>
                            <td>" . htmlspecialchars($customerName) . "</td>
                            <td>" . htmlspecialchars($row['RESCHEDULE_REFERENCE']) . "</td>
                        </tr>";
}
echo "
    </tbody>
