...
if (strpos($lot1, 'EVGM') !== false) {
 ...
 elseif (strpos($lot1, 'TOL') !== false) {
    $garden = 'Tapestry of Life';
}
elseif (strpos($lot1, 'SOL') !== false) {
    $garden = 'Solace Estates';
}
elseif (strpos($lot1, 'PE') !== false) {
    $garden = 'Piety Estates';
}
...
$stmt2->close();
    ...
    $mail->addAddress($accountEmail, $name); // Add the recipient


    $mail->isHTML(true);
    $mail->Subject = 'Interment Scheduled';
    $mail->Body = "<h1>Dear $accountName,</h1><p>We are pleased to inform you that your payment has been approved. and your interment is now scheduled.</p>";
    $mail->AltBody = "Dear $accountName, Your interment request has been scheduled successfully.";


    $mail->Body .= "<p style='color: black;'>Thank you for choosing Providence Memorial Park</p><br>";
    $mail->Body .= "<p style='color: black;'>Best regards,</p><br>";
    $mail->Body .= "<p style='color: black;'>Thank you for choosing iLocatepmpant.</p><br>";
    $mail->Body .= "<p style='color: black;'>Providence Memorial Park - Antipolo</p><br>";
    $mail->Body .= "<p style='color: black;'>https://ilocatepmpant.site | example@gmail.com | 09123456789</p><br>";
...
$theMessage = new SmsTextualMessage(
    ...
    text: "Dear $accountName, your interment request has been scheduled successfully.  Please check your email for more details.",
    from: "ProvidencePark"
);
...
$query4 = "SELECT REFERENCE_NUMBER FROM proof_of_payments WHERE INTERMENT_ORDER = ?";
...
$stmt4->execute();
$stmt4->bind_result($referencenumber);
$stmt4->fetch();
$stmt4->close();
...
$pdf->Cell(125, 5, "", 0, 0, "L");
$pdf->Cell(20, 5, "Added Tax", 0, 0, "L");
$pdf->Cell(5, 5, ": ", 0, 0, "C");

$pdf->Image('pictures/pesopeso.png', $pdf->GetX()+5, $pdf->GetY(),3.5,3.5, 'PNG');
$pdf->SetX($pdf->GetX());
$pdf->Cell($valueWidth / 3 - 15, 5, "00.00", '', 1, "C");
$pdf->Cell(125, 5, "", 0, 0, "L");
...
$totalPriceEscaped = $row['TOTAL_PRICE'];
...
VALUES ('$pdfPath', '$intermentid', 'Interment', '$accountNameEscaped', '$accountid', '$frontdesknameEscaped', '$totalPriceEscaped')";
