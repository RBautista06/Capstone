...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_FILES['proof_image']) && $_FILES['proof_image']['error'] == 0) {
        ...
        $query = "INSERT INTO tor_proof_of_payments (STATUS, PAYMENT_NUMBER, TOR_ID, PREPARED_BY, REFERENCE_NUMBER) VALUES (?, ?, ?, ?, ?)";
        ...
        $target_dir = "TORproofofpayment/$proof_id/";
        ...
        $query = "UPDATE tor_proof_of_payments SET FILE_PATH = ? WHERE PROOF_ID = ?";
        $stmt = $conn->prepare($query);
        if (!$stmt) {
            echo "Error preparing statement: " . $conn->error;
            exit;
        ...
        $updateQuery = "UPDATE payment_tor SET STATUS = 'submitted' WHERE ID = ?";
        ...
        ?>
        <script>
            alert("Payment for Transfer of Rights Submitted Successfully!!");
