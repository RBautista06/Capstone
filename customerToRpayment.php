...
<?php
 ...
 ?>
    ...
    <h1><img src="pictures/TORpayment.png" alt=""></h1>
...
<script>
    function navigatePage() {
        ...
        if (interment.checked) {
            window.location.href = 'customerintermentpayment.php?Id=<?php echo urlencode($_GET['Id']); ?>';  // Replace with the actual URL for the INTERMENT page
        } else if (transferOfRights.checked) {
            window.location.href = 'customerToRpayment.php?Id=<?php echo urlencode($_GET['Id']); ?>';  // Replace with the actual URL for the TRANSFER OF RIGHTS page
        }
    }
</script>
<?php
...
$sql = "SELECT * FROM payment_tor WHERE STATUS = 'pending' AND ACCOUNT_ID = '" . urlencode($_GET['Id']) . "'";
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    $deceasednameQuery = "SELECT TRANSFEREE_NAME,TRANSFEROR_NAME FROM transfer_of_rights WHERE ID = $INTERMENTID";
    ...
    if ($deceasednameResult) {
        ...
        if ($deceasednameRow) {
            $fullName = $deceasednameRow['TRANSFEREE_NAME'];
            $TRANSFERORfullName = $deceasednameRow['TRANSFEROR_NAME'];
        } else {
    ...
    echo "
    <tr class='clickable-row' data-href='customer_tor_paymentdetails.php?id=" . htmlspecialchars($row['ID']) . "&account_id=" .$account_id. "'>
        <td>" . htmlspecialchars($row['ID']) . "</td>
        <td>" . htmlspecialchars($row['TOR_ID']) . "</td>
        <td>" . htmlspecialchars($TRANSFERORfullName) . "</td>
