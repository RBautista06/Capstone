...
<head>
    <title>Reschedule ToR Payment</title>
...
<?php
include "dbconnection.php";
$sql = "SELECT * FROM payment_tor where STATUS ='reschedule payment'";
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    // Fetch the TRANSFEROR_NAME and TRANSFEREE_NAME from the transfer_of_rights table
    $transferQuery = "SELECT TRANSFEROR_NAME, TRANSFEREE_NAME FROM transfer_of_rights WHERE ID = $INTERMENTID";
    $transferResult = mysqli_query($conn, $transferQuery);
    if ($transferResult && mysqli_num_rows($transferResult) > 0) {
        $transferRow = mysqli_fetch_assoc($transferResult);
        $transferorName = $transferRow['TRANSFEROR_NAME'];
        $transfereeName = $transferRow['TRANSFEREE_NAME'];
    } else {
        echo "Error fetching transfer details: " . mysqli_error($conn);
        continue;
    }
    // Now echo the HTML, including the fetched reschedule fee
    echo "
    <tr class='clickable-row' data-href='customer_reschedule_tor_payment_details.php?id=" . htmlspecialchars($row['ID']) . "&account_id=" . htmlspecialchars($account_id) . "'>
        <td>" . htmlspecialchars($row['ID']) . "</td>
        <td>" . htmlspecialchars($row['TOR_ID']) . "</td>
        <td>" . htmlspecialchars($transferorName) . "</td>
        <td>" . htmlspecialchars($transfereeName) . "</td>
