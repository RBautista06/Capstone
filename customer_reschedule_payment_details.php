...
<head>
    <title>Reschedule Payment</title>
...
if ($interment_id > 0) {
    ...
    $query = "SELECT * FROM payment_interment WHERE PAYMENT_ID = ?";
    $stmt = $conn->prepare($query);
    ...
    <div class="detailstable">
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            backButton.addEventListener('click', function() {
                window.location.href = 'customer_reschedule_intermentpayment.php?Id=<?php echo urlencode($_GET['account_id']); ?>';
            });
        });
        </script>
...
<div class="details">
    ...
    <label for="">
        ...
        <button type="button" onclick="location.href='customer_reshcedule_intermentdetails.php?Id=<?php echo htmlspecialchars($interment['INTERMENT_ID']); ?>&account_id=<?php echo $account_id; ?>&payment_id=<?php echo htmlspecialchars($interment['PAYMENT_ID']); ?>'">
    ...
    <?php
include "dbconnection.php";
$interment_id1 = $interment['INTERMENT_ID'];
$query1 = "SELECT * FROM interment_forms WHERE INTERMENTFORM_ID = ?";
...
?>
        <tr>
            <th style="width: 20%;">Rescheduled Date:</th>
            <td><?php echo htmlspecialchars($interment1['DATE_OF_INTERMENT']); ?></td>
        </tr>
        <tr>
            <th style="width: 20%;">Client Code:</th>
            <td><?php echo htmlspecialchars($interment1['ACCOUNT_NUMBER']); ?></td>
        </tr>
        <tr>
            <th style="width: 20%;">Day of Interment:</th>
            <td><?php echo htmlspecialchars($interment1['DAY_OF_INTERMENT']); ?></td>
        </tr>
        <tr>
            <th style="width: 20%;">Time:</th>
            <td><?php echo htmlspecialchars($interment1['TIME']); ?></td>
        </tr>
        <tr>
    ...
    if ($reschedule_fee_result && $reschedule_fee_result->num_rows > 0) {
        ...
        $reschedule_fee = 500; // Fallback value in case of error or no result
            ...
            ?>
            <td>₱ <?php echo number_format($reschedule_fee, 2); ?></td>
        </tr>
    </table>
    <div class="totalprice">
<Label>TOTAL PRICE : </Label><span>₱ <?php echo number_format($reschedule_fee, 2); ?></span><br>
</div>
</div>
<div class="proofpayment">
    <img src="pictures/proofofpayment.png" alt="">
    <form action="reschedule_submit_payment.php?account_id=<?php echo $account_id?>" method="post" enctype="multipart/form-data">
        <table>
            <tr>
            ...
            <tr>
                <th><label for="interment_order">For Interment Order:</label></th>
                <td><input type="text" id="interment_order" name="interment_order" value="<?php echo htmlspecialchars($interment1['INTERMENTFORM_ID']); ?>" readonly></td>
            </tr>
            <tr>
