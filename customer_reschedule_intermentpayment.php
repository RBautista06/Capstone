...
<head>
    <title>Reschedule Interment Payment</title>
    <link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Material+Symbols+Outlined:opsz,wght,FILL,GRAD@20..48,100..700,0..1,-50..200">
    <link rel="stylesheet" href="customerintermentpayment.css">
...
<?php
 ...
 ?>
    ...
    <h1><img src="pictures/rescheduleintermentpayment.png" alt=""></h1>
...
<script>
    function navigatePage() {
        ...
        if (interment.checked) {
            window.location.href = 'customer_reschedule_intermentpayment.php?Id=<?php echo urlencode($_GET['Id']); ?>';  // Replace with the actual URL for the INTERMENT page
        } else if (transferOfRights.checked) {
            window.location.href = 'customer_reschedule_tor_payment.php?Id=<?php echo urlencode($_GET['Id']); ?>';  // Replace with the actual URL for the TRANSFER OF RIGHTS page
        }
    }
</script>
<?php
include "dbconnection.php";
$sql = "SELECT * FROM payment_interment where STATUS ='reschedule payment'";
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    $customerId = $row['FRONTDESK_ID'];
    $INTERMENTID = $row['INTERMENT_ID'];
    ...
    if ($customerNameResult) {
        ...
        echo "Error fetching customer name: " . mysqli_error($conn);
        continue;
    }
    $deceasednameQuery = "SELECT LASTNAME, FIRSTNAME, MIDDLENAME FROM interment_forms WHERE INTERMENTFORM_ID = $INTERMENTID";
    $deceasednameResult = mysqli_query($conn, $deceasednameQuery);
    if ($deceasednameResult) {
        $deceasednameRow = mysqli_fetch_assoc($deceasednameResult);
        if ($deceasednameRow) {
            $fullName = $deceasednameRow['FIRSTNAME'] . ' ' . $deceasednameRow['MIDDLENAME'] . ' ' . $deceasednameRow['LASTNAME'];
        } else {
            $fullName = "No name found";
        }
    } else {
        echo "Error fetching deceased name: " . mysqli_error($conn);
        continue;
    }
    // Fetch the reschedule fee from the interment_price table
    ...
    // Now echo the HTML, including the fetched reschedule fee
    echo "
    <tr class='clickable-row' data-href='customer_reschedule_payment_details.php?id=" . htmlspecialchars($row['PAYMENT_ID']) . "&account_id=" . htmlspecialchars($account_id) . "'>
        <td>" . htmlspecialchars($row['PAYMENT_ID']) . "</td>
        <td>" . htmlspecialchars($row['INTERMENT_ID']) . "</td>
        <td>" . htmlspecialchars($fullName) . "</td>
        <td>" . htmlspecialchars($customerName) . "</td>
        <td>" . htmlspecialchars($row['PAYMENT_OPTION']) . "</td>
        <td>₱ " . number_format($reschedule_fee, 2) . "</td>
    </tr>";
}
echo "
    </tbody>
